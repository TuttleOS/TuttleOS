-- ================================================================================
-- TUTTLE OS — v2.5: NAMING STANDARD SWEEP (generated from a single rename map)
-- Adopts the naming standard (see DESIGN_NOTES 'Naming Standard' section):
--   _at = timestamptz · _date = calendar fact · _due = date WE act by ·
--   _deadline = court-imposed limit · <verb>_by = staff actor ·
--   plus F2/F5/F8/F9/F10 fixes from the Field-Name Consistency Review.
-- Companion: rollback_v2.5_naming.sql (reverse renames + original functions/views).
-- ================================================================================
BEGIN;
SELECT set_config('app.staff_id', '00000000-0000-0000-0000-00000000c0de', true);

-- ---------- 1. column renames ----------
ALTER TABLE medical.affidavit_18001 RENAME COLUMN service_deadline TO service_due;
ALTER TABLE medical.affidavit_18001 RENAME COLUMN counter_deadline TO counter_affidavit_due;
ALTER TABLE medical.affidavit_18001 RENAME COLUMN counter_affidavit_received TO counter_affidavit_received_date;
ALTER TABLE litigation.affidavit_18001_party RENAME COLUMN our_service_due TO service_due;
ALTER TABLE litigation.affidavit_18001_party RENAME COLUMN served_on_date TO served_date;
ALTER TABLE litigation.affidavit_18001_party RENAME COLUMN counter_affidavit_received TO counter_affidavit_received_date;
ALTER TABLE insurance.claim_adjuster RENAME COLUMN started_at TO start_date;
ALTER TABLE insurance.claim_adjuster RENAME COLUMN ended_at TO end_date;
ALTER TABLE liens.lien_event RENAME COLUMN by_staff_id TO logged_by;
ALTER TABLE workflow.viability_review RENAME COLUMN reviewer_id TO reviewed_by;
ALTER TABLE insurance.coverage_assessment RENAME COLUMN coverage_followup_owner TO followup_owner_staff_id;
ALTER TABLE core.client_matter RENAME COLUMN current_stage TO current_stage_code;
ALTER TABLE workflow.task RENAME COLUMN detail TO description;
ALTER TABLE medical.clinical_event RENAME COLUMN detail TO description;
ALTER TABLE litigation.service_of_process RENAME COLUMN service_id TO service_of_process_id;
ALTER TABLE analytics.closed_case_snapshot RENAME COLUMN case_type TO case_type_code;
ALTER TABLE core.incident_group RENAME COLUMN preservation_letter_sent TO preservation_letter_sent_date;
ALTER TABLE core.intake_lead RENAME COLUMN non_engagement_letter_sent TO non_engagement_letter_sent_date;
ALTER TABLE core.limitations_analysis RENAME COLUMN charter_notice_sent TO charter_notice_sent_date;
ALTER TABLE core.limitations_analysis RENAME COLUMN ttca_notice_sent TO ttca_notice_sent_date;
ALTER TABLE core.limitations_analysis RENAME COLUMN medmal_notice_sent TO medmal_notice_sent_date;
ALTER TABLE insurance.claim RENAME COLUMN date_reported TO reported_date;
ALTER TABLE insurance.claim RENAME COLUMN lor_sent TO lor_sent_date;
ALTER TABLE insurance.claim RENAME COLUMN policy_limits_disclosed TO policy_limits_disclosed_date;
ALTER TABLE insurance.claim RENAME COLUMN policy_limits_request_sent TO policy_limits_request_sent_date;
ALTER TABLE insurance.policy RENAME COLUMN dec_sheet_received TO dec_sheet_received_date;
ALTER TABLE insurance.policy RENAME COLUMN dec_sheet_requested TO dec_sheet_requested_date;
ALTER TABLE liens.medicare_detail RENAME COLUMN rights_and_responsibilities_received TO rights_and_responsibilities_received_date;
ALTER TABLE litigation.court_case RENAME COLUMN bifurcation_motion_filed TO bifurcation_motion_filed_date;
ALTER TABLE litigation.court_case RENAME COLUMN remand_motion_filed TO remand_motion_filed_date;
ALTER TABLE litigation.deposition RENAME COLUMN notice_served TO notice_served_date;
ALTER TABLE litigation.deposition RENAME COLUMN transcript_received TO transcript_received_date;
ALTER TABLE litigation.discovery_set RENAME COLUMN responses_served TO responses_served_date;
ALTER TABLE litigation.discovery_set RENAME COLUMN extension_agreed_to TO extension_agreed_date;
ALTER TABLE litigation.expert RENAME COLUMN challenge_motion_filed TO challenge_motion_filed_date;
ALTER TABLE litigation.expert RENAME COLUMN report_served TO report_served_date;
ALTER TABLE litigation.lit_party RENAME COLUMN answer_filed TO answer_filed_date;
ALTER TABLE litigation.motion RENAME COLUMN reply_filed TO reply_filed_date;
ALTER TABLE litigation.motion RENAME COLUMN response_filed TO response_filed_date;
ALTER TABLE litigation.scheduling_order RENAME COLUMN expert_designation_plaintiff TO expert_designation_plaintiff_date;
ALTER TABLE litigation.scheduling_order RENAME COLUMN expert_designation_defense TO expert_designation_defense_date;
ALTER TABLE litigation.scheduling_order RENAME COLUMN discovery_close TO discovery_close_date;
ALTER TABLE litigation.scheduling_order RENAME COLUMN pretrial_disclosures_due TO pretrial_disclosures_deadline;
ALTER TABLE litigation.service_of_process RENAME COLUMN citation_requested TO citation_requested_date;
ALTER TABLE litigation.service_of_process RENAME COLUMN citation_issued TO citation_issued_date;
ALTER TABLE litigation.service_of_process RENAME COLUMN rule_106_motion_filed TO rule_106_motion_filed_date;
ALTER TABLE litigation.service_of_process RENAME COLUMN rule_106_order_signed TO rule_106_order_signed_date;
ALTER TABLE litigation.trial_witness RENAME COLUMN subpoena_issued TO subpoena_issued_date;
ALTER TABLE litigation.trial_witness RENAME COLUMN subpoena_served TO subpoena_served_date;
ALTER TABLE medical.provider_contact_log RENAME COLUMN next_appointment TO next_appointment_date;
ALTER TABLE medical.treatment_episode RENAME COLUMN first_visit TO first_visit_date;
ALTER TABLE medical.treatment_episode RENAME COLUMN last_visit TO last_visit_date;
ALTER TABLE medical.treatment_episode RENAME COLUMN projected_completion TO projected_completion_date;
ALTER TABLE resolution.demand RENAME COLUMN response_received TO response_received_date;
ALTER TABLE workflow.public_records_request RENAME COLUMN response_received TO response_received_date;
ALTER TABLE workflow.deadline RENAME COLUMN satisfied_at TO satisfied_date;

COMMENT ON COLUMN litigation.scheduling_order.pretrial_disclosures_deadline IS
  'v2.5: renamed from pretrial_disclosures_due — court-imposed dates use _deadline; _due is reserved for obligations we must perform.';
COMMENT ON COLUMN workflow.task.description IS 'v2.5: renamed from detail — description is the standard long-text field name.';

-- ---------- 2. function patches (token-rewritten from live definitions) ----------
CREATE OR REPLACE FUNCTION analytics.finalize_matter_close(p_matter uuid)
 RETURNS void
 LANGUAGE sql
AS $function$
INSERT INTO analytics.closed_case_snapshot (
  client_matter_id, case_type_code, county, venue_county, case_manager,
...
AS $function$
  ...
  (SELECT 'client_matter', m.client_matter_id,
          ...
          coalesce(m.matter_number,'') || ' · ' || m.current_stage_code,
...
AS $function$
BEGIN
  IF TG_OP = 'INSERT' OR NEW.current_stage_code IS DISTINCT FROM OLD.current_stage_code THEN
...
BEGIN
  IF TG_OP = 'INSERT' OR NEW.current_stage_code IS DISTINCT FROM OLD.current_stage_code THEN
    ...
    VALUES (NEW.client_matter_id, NEW.current_stage_code);
  END IF;
  RETURN NULL;
END $function$;
...
BEGIN
  IF NEW.lor_sent_date IS NOT NULL
     AND (TG_OP = 'INSERT' OR OLD.lor_sent_date IS DISTINCT FROM NEW.lor_sent_date)
...
BEGIN
  ...
  FOR r IN SELECT * FROM (VALUES
      ...
      ('Plaintiff expert designation (DCO)', so.expert_designation_plaintiff_date, NULL),
      ('Defense expert designation (DCO)',   so.expert_designation_defense_date,   NULL),
  ...
  IF so.discovery_close_date IS NOT NULL THEN
    UPDATE litigation.court_case
       SET discovery_period_end = so.discovery_close_date,
...
DECLARE
  v_matter uuid;
BEGIN
  IF NEW.answer_filed_date IS NULL THEN RETURN NEW; END IF;
  IF TG_OP = 'UPDATE' AND OLD.answer_filed_date IS NOT DISTINCT FROM NEW.answer_filed_date THEN RETURN NEW; END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM workflow.deadline d
    ...
    SELECT NEW.lit_party_id, v_matter, 'affidavit_18001_serve_90',
           ...
           'answer filed', NEW.answer_filed_date,
           NEW.answer_filed_date + 90, NEW.answer_filed_date + 90, 'rule', false
  ...
  IF NOT EXISTS (SELECT 1 FROM workflow.deadline d
    ...
    SELECT NEW.lit_party_id, v_matter, 'affidavit_18001_counter_120',
           ...
           'answer filed', NEW.answer_filed_date,
           NEW.answer_filed_date + 120, NEW.answer_filed_date + 120, 'rule', false
    WHERE NOT EXISTS (SELECT 1 FROM workflow.deadline d
                       WHERE d.entity_id = NEW.lit_party_id AND d.status = 'pending'
                         AND d.rule_code = 'affidavit_18001_counter_120');
  END IF;
...
-- ---------- 3. view rebuilds (output columns follow the new names) ----------
...
CREATE VIEW resolution.v_demand_readiness WITH (security_invoker=on) AS
 SELECT m.client_matter_id,
  ...
  WHERE m.deleted_at IS NULL AND (m.current_stage_code = ANY (ARRAY['records'::text, 'demand'::text]))
...
CREATE VIEW workflow.v_stalled_cases WITH (security_invoker=on) AS
 SELECT client_matter_id,
    ...
    current_stage_code = 'records'::text AND (EXISTS ( SELECT 1
           FROM medical.treatment_episode te
    ...
    (EXISTS ( SELECT 1
           FROM property.pd_claim pd
             JOIN property.vehicle v ON v.vehicle_id = pd.vehicle_id
          WHERE v.incident_group_id = m.incident_group_id AND (pd.status <> ALL (ARRAY['resolved'::text, 'n_a'::text])) AND (m.current_stage_code = ANY (ARRAY['demand'::text, 'negotiation'::text])))) AS flag_pd_unresolved,
    (EXISTS ( SELECT 1
           FROM resolution.demand d
          WHERE d.client_matter_id = m.client_matter_id AND d.sent_date IS NOT NULL AND d.response_received_date IS NULL AND d.response_due < CURRENT_DATE)) AS flag_demand_response_overdue,
    (EXISTS ( SELECT 1
    ...
    sol_date IS NOT NULL AND sol_date < (CURRENT_DATE + 120) AND current_stage_code <> 'closed'::text AS flag_sol_within_120d
   FROM core.client_matter m
  WHERE deleted_at IS NULL AND representation_status = 'active'::text AND current_stage_code <> 'closed'::text;
